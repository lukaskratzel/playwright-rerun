...
export async function rafraf(target: Page | Frame, count = 1) {
  for (let i = 0; i < count; i++) {
    await target.evaluate(async () => {
      const builtinRequestAnimationFrame = (window as any).__pwFakeTimers?.builtin.requestAnimationFrame || requestAnimationFrame;
      await new Promise(f => builtinRequestAnimationFrame(() => builtinRequestAnimationFrame(f)));
    });
  }
}
