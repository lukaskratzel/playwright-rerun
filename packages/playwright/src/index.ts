...
const playwrightFixtures: Fixtures<TestFixtures, WorkerFixtures> = ({
  ...
  playwright: [async ({}, use) => {
    ...
    const instrumentation: TestLifecycleInstrumentation = {
      onTestBegin: async () => {
        artifactsRecorder = new ArtifactsRecorder(playwright, tracing().artifactsDir());
    ...
    if (currentTestInfo()) {
      // Setup of "playwright" fixture happens after the first test starts.
    ...
    if (currentTestInfo()) {
      // Teardown of "playwright" fixture might happen right before the failing test ends
